        ...
        while flag:
            for line in self.pack_products_ids:
                if line.qty_available >= line.quantity * total_quantity:
                    ...
                    product_to_check = line.product_id or (line.base_product_id.product_variant_ids[0] if line.base_product_id and line.base_product_id.product_variant_ids else False)
                    if not product_to_check or product_to_check.type != 'product':
                        continue
                    flag = 0
                    break
            if flag:
